...
def origami_inference_forward(data, policy, device, use_tactile):
    ...
    # normalizer that training used (passed in by the caller). When the
    # checkpoint was trained unnormalized the normalizer is identity, so this
...
# ──────────────────────────────────────────────────────────────────────
def load_training_normalizer(ckpt_path, device):
    """Rebuild the exact normalizer a checkpoint was trained with.

    Training writes normalizer_config.json next to the checkpoint holding the
    feature modes and the raw stats they were derived from. Rebuilding from
    those reproduces the grouping and degenerate-dim guard deterministically.

    Returns (normalizer, meta) where normalizer is None if the checkpoint was
    trained without normalization. Raises if the sidecar is missing, because
    guessing here silently produces either garbage inputs or joint commands in
    the wrong units.
    """
    ...
    sidecar = os.path.join(ckpt_dir, "normalizer_config.json")
    if not os.path.exists(sidecar):
        raise FileNotFoundError(
            f"{sidecar} not found.\n"
            f"Inference must use the same normalization as training. Without this file\n"
            f"there is no way to know whether the checkpoint expects normalized inputs,\n"
            f"or whether its outputs are normalized joint angles rather than radians.\n"
            f"Checkpoints trained before this file existed were all unnormalized: if you\n"
            f"are certain that applies, pass --assume_unnormalized."
        )
    ...
    if cfg.get("disable_normalization", False):
        print("[Normalizer] checkpoint was trained WITHOUT normalization (identity)")
        return None, cfg
    ...
    normalizer = OriNormalizer(
        ...
        device=device,
        degenerate_spread=cfg.get("degenerate_spread", 1e-3),
        clip=cfg.get("clip"),
    )
    print("[Normalizer] rebuilt from", sidecar)
    print(normalizer.describe())
    if not cfg.get("image_norm", True):
        os.environ["ORI_IMAGE_NORM"] = "0"
        print("[Normalizer] checkpoint trained WITHOUT ImageNet image normalization "
              "-> ORI_IMAGE_NORM=0")
...
def run_episode_inference(dataset, policy, device, use_tactile, episode_idx,
    ...
    for seg_idx, start in enumerate(chunk_starts):
        ...
        if normalizer is not None:
            _gt = normalizer.denormalize("action", _gt.float())
        gt_actions = _gt.cpu().numpy()  # [1, T, D] radians
        ...
        # the robot as radians would command every joint to roughly +/-1 rad.
        if normalizer is not None:
            a_hat = normalizer.denormalize("action", a_hat.float())
        pred_actions = a_hat.cpu().numpy()  # [1, T, D] radians
...
def main():
    ...
    if getattr(args, "assume_unnormalized", False):
        normalizer = None
    ...
    for batch_idx, raw_batch in enumerate(dataloader):
        ...
        if normalizer is not None:
            _gt = normalizer.denormalize("action", _gt.float())
        gt_actions = _gt.cpu().numpy()  # [B, T, D] radians
        ...
        if normalizer is not None:
            a_hat = normalizer.denormalize("action", a_hat.float())
        pred_actions = a_hat.cpu().numpy()  # [B, T, D] radians
